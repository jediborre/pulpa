"""Train V7 State-of-the-Art predictors for quarter winners (Q3, Q4).

V7 upgrades to CatBoost and XGBoost(enable_categorical=True), natively processing
raw un-bucketed categorical strings (huge cardinality) to maximize SOTA extraction.
"""
...
OUT_DIR = ROOT / "training" / "model_outputs_v7"
...
def _build_samples(db_path: Path) -> list[MatchSample]:
    ...
    # CatBoost handles raw categories natively, no need to group low frequency items into OTHER buckets.
...
def _train_target(samples: list[MatchSample], target_name: str) -> dict:
    ...
    # XGBoost requires categorical columns to specifically be of dtype 'category'
    for col in CATEGORICAL:
        df_x[col] = df_x[col].astype('category')

    x_train = df_x.iloc[:n_train]
    x_test = df_x.iloc[n_train:]
    y_train = y[:n_train]
    y_test = y[n_train:]
    ...
    agreement = sum(
        ...
        if len(
            {
                int(proba_map["catboost"][idx] >= 0.5),
                int(proba_map["xgb_cat"][idx] >= 0.5),
            }
        )
        == 1
    )
