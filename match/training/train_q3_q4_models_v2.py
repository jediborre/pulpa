"""Train V2 multi-model predictors for quarter winners (Q3, Q4).

V2 adds league/team bucket features and extended evaluation metrics.
"""
...
def _build_samples(db_path: Path) -> list[MatchSample]:
    ...
    for row in rows:
        ...
        features_q3.update(_pbp_stats_upto(pbp, ["Q1", "Q2"]))
        ...
        features_q4.update(_pbp_stats_upto(pbp, ["Q1", "Q2", "Q3"]))
...
def main() -> None:
    ...
    print("[train-v2] done")
    print(f"[train-v2] samples_complete={len(samples)}")
    print(f"[train-v2] q3_rows={q3_result['n_rows']}")
    print(f"[train-v2] q4_rows={q4_result['n_rows']}")
    print(f"[train-v2] outputs={OUT_DIR}")
