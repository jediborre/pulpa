# V17 - Engine de prediccion de cuarto ganador
...
py -V:3.11 -m venv training/v17/.venv311
training/v17/.venv311/Scripts/python.exe -m pip install --upgrade pip wheel setuptools
training/v17/.venv311/Scripts/python.exe -m pip install numpy pandas scikit-learn xgboost catboost joblib tqdm matplotlib
training/v17/.venv311/Scripts/python.exe -m pip install --index-url https://download.pytorch.org/whl/cpu torch
training/v17/.venv311/Scripts/python.exe -m pip install "timesfm[torch]"
```
...
Se re-entreno v17 con `TIMESFM_ENABLED=False` usando los **mismos CLI args,
...
reusable en `training/v17/ab_compare.py`.
...
- `reports/test_roi_v17_TFM.json`, `_NOTFM.json`, `_CHRONOS.json`
...
2. **Chronos ocupa el lugar intermedio** (ROI +5%, hit 75%) entre TimesFM
   ...
   tarda 4Ã— mas (3417 s vs 852 s, principalmente por la precomputacion).
...
./training/v17/.venv311/Scripts/python.exe -m training.v17.cli test-roi --odds 1.40 --min-bets 5 --top 30
...
./training/v17/.venv311/Scripts/python.exe -m training.v17.cli eval
```
