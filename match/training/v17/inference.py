"""
inference.py - Motor de inferencia V17 (carga una vez, inferencia multiple).
...
from training.v17 import config, dataset as ds, features as feat, gates, league_overrides
...
class V15Engine:
    ...
    def load(cls, summary_path: Path | str = SUMMARY_PATH) -> "V15Engine":
        ...
        for m in summary.get("models", []):
            ...
            try:
                ...
                print(f"[v17] error cargando modelo {league}/{target}: {e}")
                continue
    ...
    def predict(
        ...
        snapshot_minute: float | None = None,
    ) -> Prediction:
        """
        Predice para un partido en vivo o historico.
        ...
        Args:
            ...
            snapshot_minute: si se provee, ignora el cutoff de config
        """
        debug = Debug(
            match_id=match_id, target=target, league=league,
            league_slug=ds.slugify_league(league),
            model_found=False,
            model_version=self.summary.get("version", "v17"),
        )
        ...
        cutoff = snapshot_minute if snapshot_minute is not None else (
            config.Q3_GRAPH_CUTOFF if target == "q3" else config.Q4_GRAPH_CUTOFF
        )
