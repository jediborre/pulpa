...
def _load(name: str) -> dict | None:
    p = RPT / f"test_roi_v17_{name}.json"
    if not p.exists():
        return None
...
def _load_summary(name: str) -> dict:
    p = OUT / f"training_summary_v17_{name}.json"
    if not p.exists():
        return {}
    return json.loads(p.read_text(encoding="utf-8"))
...
W = 92
print("=" * W)
print(" " * 20 + "V17 A/B/C REPORT  -  TimesFM vs Chronos vs Holt baseline")
print("=" * W)
print()
...
print("  - Cronos entrenÃ³ 87 pares (vs 46 de TimesFM) porque no filtra por pbp minimo.")
