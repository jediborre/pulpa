"""
test_roi.py - Script de prueba de ROI V17 con salida legible.
...
Uso:
    python -m training.v17.cli test-roi
    python -m training.v17.cli test-roi --odds 1.50 --top 10
    python -m training.v17.test_roi   # directo

Salidas:
  reports/test_roi_v17.csv  (tabla completa por liga+target)
  reports/test_roi_v17.json (resumen estructurado para consumir desde API)
"""
...
from training.v17.inference import V15Engine, SUMMARY_PATH
...
def run(
    ...
    if not SUMMARY_PATH.exists():
        print(f"{C.RED}[error] no hay modelos entrenados. "
              f"Corre: python -m training.v17.cli train{C.RESET}")
        return {}
    ...
    print(f"{C.BOLD}{C.CYAN}[v17/test-roi] odds={odds}  "
          f"break-even={1/odds:.4f}  target={config.TARGET_HIT_RATE:.0%}{C.RESET}")
    print(f"[v17/test-roi] cargando engine...")
    engine = V15Engine.load(SUMMARY_PATH)

    print(f"[v17/test-roi] cargando muestras y split...")
    samples, _ = ds.build_samples(use_cache=True, verbose=False)
    with open(SUMMARY_PATH, "r", encoding="utf-8") as f:
        summary = json.load(f)
    run_params = summary.get("run_params") or {}
    splits = ds.split_temporal(
        samples,
        train_days=run_params.get("train_days", config.TRAIN_DAYS),
        val_days=run_params.get("val_days", config.VAL_DAYS),
        cal_days=run_params.get("cal_days", config.CAL_DAYS),
        holdout_days=run_params.get("holdout_days", config.HOLDOUT_DAYS),
        train_end_date=run_params.get("train_end_date"),
        val_end_date=run_params.get("val_end_date"),
        cal_end_date=run_params.get("cal_end_date"),
    )
    holdout = [
    ...
    if not holdout:
        ...
        return {}
    print(f"[v17/test-roi] {len(holdout)} muestras de holdout "
    ...
    print(f"[v17/test-roi] corriendo inferencia...")
    ...
    bankroll = float(initial_bank)
    settled_bets = 0
    settled_wins = 0
    for row in sorted(all_preds, key=lambda r: (r["date"], r["match_id"], r["target"])):
        if row["signal"] not in ("BET_HOME", "BET_AWAY"):
            continue
        if bankroll < bet_size:
            break
        settled_bets += 1
        win = (
            (row["signal"] == "BET_HOME" and row["true_winner"] == 1)
            or (row["signal"] == "BET_AWAY" and row["true_winner"] == 0)
        )
        if win:
            bankroll += bet_size * (odds - 1.0)
            settled_wins += 1
        else:
            bankroll -= bet_size
    bankroll_pnl = bankroll - initial_bank
    bankroll_roi = bankroll_pnl / initial_bank if initial_bank else None
    ...
    # Imprimir
    _print_header(odds, len(holdout), total_preds, initial_bank, bet_size)
    _print_global(global_roi, global_hit, total_bets, total_wins, total_pnl, odds)
    _print_bankroll(initial_bank, bankroll, bankroll_pnl, bankroll_roi, settled_bets, settled_wins)
    ...
    out_json = REPORTS_DIR / "test_roi_v17.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, default=_jsonable)
    print(f"\n[v17/test-roi] resumen -> {out_json}")

    if emit_csv:
        out_csv = REPORTS_DIR / "test_roi_v17.csv"
        ...
        print(f"[v17/test-roi] csv     -> {out_csv}")
...
def _print_header(odds: float, n_holdout: int, n_preds: int, initial_bank: float, bet_size: float) -> None:
    print()
    print(f"{C.BOLD}{'=' * 92}{C.RESET}")
    print(f"{C.BOLD}{C.CYAN}  V17 - ROI TEST REPORT{C.RESET}")
    print(f"{C.BOLD}{'=' * 92}{C.RESET}")
    ...
    print(f"  stake fijo            : {bet_size:.2f}")
...
def _print_bankroll(initial_bank, final_bank, pnl, roi_on_bank, n_bets, wins) -> None:
    print(f"{C.BOLD}BANKROLL (stake fijo por apuesta){C.RESET}")
    print(f"  apuestas liquidadas   : {n_bets}")
    print(f"  ganadas               : {wins}")
    print(f"  bank inicial          : {initial_bank:.2f}")
    print(f"  bank final            : {final_bank:.2f}")
    sign = "+" if pnl >= 0 else ""
    color = _roi_color(roi_on_bank if roi_on_bank is not None else 0.0)
    print(f"  P&L bank              : {color}{sign}{pnl:.2f}{C.RESET}")
    if roi_on_bank is not None:
        print(f"  ROI sobre bank        : {color}{roi_on_bank:+.4f}{C.RESET}")
    print(f"{'-' * 92}")
...
def _print_hint(global_roi, portfolio_roi) -> None:
    ...
    if portfolio_roi is None:
        ...
        print(f"  - Correr {C.BOLD}python -m training.v17.cli plots{C.RESET} "
...
if __name__ == "__main__":
    ...
    ap.add_argument("--initial-bank", type=float, default=1000.0)
    ap.add_argument("--bet-size", type=float, default=100.0)
