...
1. **Lunes por la manana (antes del arranque de la semana competitiva)**:
  1. Actualizar la base de datos con los partidos de la semana anterior
    (scrapers de v15 / v17, lo que este activo).
  2. Desde `match/`:
    ```powershell
     cd match
     $env:PYTHONIOENCODING = "utf-8"
     ./training/v17/.venv311/Scripts/python.exe -u -m training.v17.cli train `
  ...
  3. Revisar `reports/training_run_tfm.log`. Buscar:
    - Linea `[v17/timesfm] backend=timesfm cache_loaded=N`. Si dice
...
2. **Evaluar el estado del run**:
  ```powershell
   ./training/v17/.venv311/Scripts/python.exe training/v17/reports/_summarize_prod.py
  ```
...
3. **Test de ROI en holdout**:
  ```powershell
   ./training/v17/.venv311/Scripts/python.exe -m training.v17.cli test-roi `
...
4. **Regenerar overrides**:
  ```powershell
   ./training/v17/.venv311/Scripts/python.exe -m training.v17.reports._auto_overrides
  ```
...
| Todas las apuestas se concentran en 1 liga    | `portfolio.leagues` en `test_roi_v17.json`  | Diversificar: relajar thresholds en ligas con val_roi > 0 pero n_bets bajo.                                                                                        |
...
`training/v17/model_outputs_tfm_baseline/` por si se detectan regresiones.
Reversion 1-comando:

```powershell
Move-Item training/v17/model_outputs training/v17/model_outputs_pruned
Move-Item training/v17/model_outputs_tfm_baseline training/v17/model_outputs
...
# -> ya esta guardada en reports/test_roi_v17_TFM.json y model_outputs_tfm_backup/

# Variante OFF
# 1. Editar config.py: TIMESFM_ENABLED = False
# 2. Apartar modelos actuales:
Move-Item training/v17/model_outputs/*.joblib training/v17/model_outputs_tfm_backup/
# 3. Re-entrenar:
./training/v17/.venv311/Scripts/python.exe -u -m training.v17.cli train `
  --train-days 72 --val-days 15 --cal-days 3 --holdout-days 7 `
  --min-samples-train 150 --min-samples-val 40
# 4. Evaluar:
./training/v17/.venv311/Scripts/python.exe -u -m training.v17.cli test-roi
# 5. Guardar como NOTFM y generar comparativo:
Copy-Item training/v17/reports/test_roi_v17.json training/v17/reports/test_roi_v17_NOTFM.json
./training/v17/.venv311/Scripts/python.exe training/v17/ab_compare.py > training/v17/reports/ab_compare_report.txt
...
+5.00%, hit 75.00%, 8 bets â€” mejor ROI que TimesFM en este holdout pero
no significativo (n<12). Modelo por defecto: `chronos-bolt-tiny` (8M params,
~4Ã— mas rapido que TimesFM en batch, 4Ã— mas lento en precompute por tener
mas series distintas). Integrado en `chronos_features.py`.

```bash
# Instalar en el venv 3.11
./training/v17/.venv311/Scripts/python.exe -m pip install chronos-forecasting
```
...
3. `test-roi` -> guardar como `test_roi_v17_CHRONOS.json`.
...
- Copiar `training_summary_v17.json` a
`training_summary_v17_PROD_YYYYMMDD.json` como snapshot de referencia.
