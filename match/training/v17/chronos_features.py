...
def _active_model_name() -> str:
    try:
        from training.v17 import config as _cfg
...
def _get_chronos_model(model_name: Optional[str] = None):
    ...
    with _model_lock:
        ...
        try:
            ...
            print(f"[v17/chronos] fallo al cargar el modelo {name}: {e}")
            _chronos_model = None
            return None
...
def _forecast_one(series: np.ndarray, horizon: int, num_samples: int = 20) -> dict[str, float]:
    ...
    try:
        ...
        print(f"[v17/chronos] fallo singleton: {e}")
        return _empty_features()
...
def _forecast_batch(
    ...
    try:
        ...
        print(f"[v17/chronos] fallo batch ({e}), caigo a iteracion")
