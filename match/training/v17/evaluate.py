"""
evaluate.py - Backtest y reportes de V17.

Genera:
- eval_report_v17.json   : metricas por (liga, target), ROI, curvas de threshold
- eval_predictions_v17.csv: todas las predicciones del holdout con contexto
...
CLI: python -m training.v17.evaluate --odds 1.40 --out training/v17/reports/
"""
...
from training.v17.inference import V15Engine, MODEL_DIR, SUMMARY_PATH
...
def run_backtest(
    ...
    snapshot_only_prod: si True usa solo los cutoffs de produccion
      (Q3=22, Q4=31) â€” el escenario live real.
    """
    REPORTS_DIR.mkdir(exist_ok=True, parents=True)
    print("[v17/eval] cargando engine...")
    engine = V15Engine.load(SUMMARY_PATH)

    print("[v17/eval] cargando muestras...")
    ...
    print(f"[v17/eval] {len(target_samples)} muestras de evaluacion")
...
def _write_report(preds: list[dict], report: dict) -> None:
    REPORTS_DIR.mkdir(exist_ok=True, parents=True)
    # JSON
    json_path = REPORTS_DIR / "eval_report_v17.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, default=_jsonable)
    print(f"[v17/eval] -> {json_path}")

    # CSV predicciones
    csv_path = REPORTS_DIR / "eval_predictions_v17.csv"
    if preds:
    ...
    print(f"[v17/eval] curvas de threshold -> {curves_dir}")
...
def main():
    ap = argparse.ArgumentParser(description="V17 evaluator")
