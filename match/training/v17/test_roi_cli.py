...
Uso:
  python -m training.v17.test_roi_cli --holdout-last-days 7 --suffix BASELINE_h7
"""
...
from training.v17 import config, test_roi as troi
from training.v17 import dataset as ds
...
def main():
    ...
    ap.add_argument("--suffix", default=None,
                    help="Sufijo para outputs (test_roi_v17_<suffix>.json)")
    args = ap.parse_args()
    ...
    if args.suffix:
        src = Path(__file__).parent / "reports" / "test_roi_v17.json"
        dst = Path(__file__).parent / "reports" / f"test_roi_v17_{args.suffix}.json"
        if src.exists():
            import shutil
            shutil.copy(src, dst)
