"""
train.py - Orquestador de entrenamiento V16.
...
Salida: training/v16/model_outputs/
  - training_summary_v16.json       (config + metricas + thresholds por liga)
...
from training.v16 import config, dataset as ds, features as feat, models as M
...
def _precompute_tfm_cache(
    ...
    try:
        if backend == "chronos":
            from training.v16 import chronos_features as _fc
            is_available = _fc.is_chronos_available()
            diag_key = "chronos"
            pkg_label = "chronos"
        else:
            from training.v16 import timesfm_features as _fc
            is_available = _fc.is_timesfm_available()
            diag_key = "timesfm"
            pkg_label = "timesfm"
    except Exception as e:
        print(f"[v16] aviso: no pude importar forecast module ({e}). "
              f"build_features se servira sin cache.")
        return None

    if not is_available:
        if verbose:
            print(f"[v16/{diag_key}] paquete `{pkg_label}` no disponible; "
    ...
    if verbose:
        diag = _fc.diagnostic()
        print(f"[v16/{diag_key}] backend={diag['active_backend']} "
    ...
    if verbose:
        print(f"[v16/{diag_key}] cache poblado: {len(cache)} entries")
    return cache
...
def run_training(
    ...
    if getattr(config, "USE_LEAGUE_ACTIVATION", True):
        try:
            ...
            print(f"[v16] selector inteligente de ligas: "
                  f"{before} -> {len(samples)} muestras "
                  f"({len(active_set)} activas / {len(dormant)} dormidas). "
                  f"Reporte en model_outputs/league_assessments_v16.json")
            dataset_analyzer.save_report(assessments)
        except Exception as e:
            print(f"[v16] aviso: dataset_analyzer fallo ({e}), "
    ...
    if active_days and active_days > 0:
        ...
        if parsed:
            ...
            print(f"[v16] filtro ligas activas (ultimos {active_days}d): "
                  f"{before} -> {len(samples)} muestras  "
                  f"({len(active_leagues)} ligas activas)")

    # 2. Split
    print(f"[v16] split temporal "
    ...
    print(f"[v16] {len(candidate_leagues)} ligas candidatas "
    ...
    try:
        ...
        print(f"[v16] aviso: no pude persistir {_backend} cache ({e})")
    ...
    print(f"[v16] summary -> {SUMMARY_PATH}")
    print(f"[v16] trained {len(league_results)} (league,target) pairs, "
    ...
    try:
        ...
        print(f"[v16] aviso: generacion de plots fallo ({e}). "
              "Correr `python -m training.v16.cli plots` manualmente.")
    return summary
