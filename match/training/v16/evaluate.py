"""
evaluate.py - Backtest y reportes de V16.

Genera:
- eval_report_v16.json   : metricas por (liga, target), ROI, curvas de threshold
- eval_predictions_v16.csv: todas las predicciones del holdout con contexto
...
CLI: python -m training.v16.evaluate --odds 1.40 --out training/v16/reports/
"""
...
from training.v16.inference import V15Engine, MODEL_DIR, SUMMARY_PATH
...
def run_backtest(
    ...
    print("[v16/eval] cargando engine...")
    engine = V15Engine.load(SUMMARY_PATH)

    print("[v16/eval] cargando muestras...")
    ...
    print(f"[v16/eval] {len(target_samples)} muestras de evaluacion")
...
def _write_report(preds: list[dict], report: dict) -> None:
    REPORTS_DIR.mkdir(exist_ok=True, parents=True)
    # JSON
    json_path = REPORTS_DIR / "eval_report_v16.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, default=_jsonable)
    print(f"[v16/eval] -> {json_path}")

    # CSV predicciones
    csv_path = REPORTS_DIR / "eval_predictions_v16.csv"
    if preds:
    ...
    print(f"[v16/eval] curvas de threshold -> {curves_dir}")
...
def main():
    ap = argparse.ArgumentParser(description="V16 evaluator")
