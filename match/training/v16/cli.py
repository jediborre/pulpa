"""
cli.py - Entrypoint unificado para V16.

Ejemplos:

  # Entrenar todo:
  python -m training.v16.cli train
  python -m training.v16.cli train --no-cache

  # Backtest sobre holdout con odds 1.40 (default):
  python -m training.v16.cli eval
  ...
  echo '{ "match_id": "abc", "target": "q3", "league": "NBA",
    ...
    python -m training.v16.cli infer

  # Listar ligas entrenadas y su ROI de holdout:
  python -m training.v16.cli leagues

  # Exportar config + overrides activos:
  python -m training.v16.cli config
"""
...
def _cmd_train(args):
    from training.v16 import train
    train.run_training(
...
def _cmd_eval(args):
    from training.v16 import evaluate
...
def _cmd_infer(args):
    from training.v16.inference import V15Engine
    raw = sys.stdin.read()
    if not raw.strip():
...
def _cmd_leagues(args):
    from training.v16.inference import SUMMARY_PATH
...
def _cmd_test_roi(args):
    from training.v16 import test_roi
    test_roi.run(
        odds=args.odds,
        top_n=args.top,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="v16", description="V16 CLI")
