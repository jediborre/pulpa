"""
cli_menu.py - CLI interactivo con menus comprensibles para V16.

Ejecucion:

    python -m training.v16.cli_menu
...
Si preferis una invocacion directa por argumentos, segui usando `cli.py`:
    ...
    python -m training.v16.cli train --train-days 72 ...
"""
...
def _render_menu(title: str, items: list[MenuItem], extra_info: str = ""):
    _clear()
    _banner("V16 - Modelo live de apuestas NBA/basket", title)
    if extra_info:
        print()
        print(extra_info)
    print()
...
def _run_cli(args: list[str]):
    print(f"  {C_DIM}$ python -m training.v16.cli {' '.join(args)}{C_RESET}")
    print()
    env = os.environ.copy()
    env.setdefault("PYTHONIOENCODING", "utf-8")
    try:
        subprocess.run(
            [sys.executable, "-u", "-m", "training.v16.cli", *args],
            cwd=str(ROOT),
            env=env,
            check=False,
        )
...
def action_train_baseline():
    ...
    print(f"  {C_DIM}Config original de v16:{C_RESET}")
...
def action_generate_plots():
    ...
    _run_cli(["plots"])
    plots_dir = ROOT / "training" / "v16" / "model_outputs" / "plots"
    if plots_dir.exists():
        print()
...
def action_summarize_prod():
    ...
    script = ROOT / "training" / "v16" / "reports" / "_summarize_prod.py"
    if script.exists():
...
def action_infer_sample():
    ...
    env.setdefault("PYTHONIOENCODING", "utf-8")
    proc = subprocess.run(
        [sys.executable, "-u", "-m", "training.v16.cli", "infer"],
...
def action_show_prod_summary():
    ...
    path = ROOT / "training" / "v16" / "model_outputs" / "training_summary_v16.json"
    if not path.exists():
...
def action_scan_db():
    ...
    script = ROOT / "training" / "v16" / "reports" / "_scan_db.py"
    if script.exists():
        env = os.environ.copy()
        env.setdefault("PYTHONIOENCODING", "utf-8")
...
def action_run_sweep():
    ...
    if confirm.lower() not in ("s", "si", "y", "yes"):
        return
    script = ROOT / "training" / "v16" / "reports" / "_sweep.ps1"
...
def action_open_docs():
    ...
    docs = {
        "readme": ROOT / "training" / "v16" / "README.md",
        "recomendaciones": ROOT / "training" / "v16" / "RECOMENDACIONES.md",
        "roadmap": ROOT / "training" / "v16" / "ROADMAP.md",
    }
...
def submenu_train():
    items = [
        ...
        MenuItem("3", "Entrenar BASELINE (referencia)",
                 ...
                 "config original v16 con holdout, para comparar"),
