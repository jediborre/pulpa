...
CACHE_PATH = MODEL_OUTPUTS_DIR / "samples_cache_v16.json"
...
def build_samples(
    ...
    if use_cache and CACHE_PATH.exists():
        if verbose:
            print(f"[v16] usando cache {CACHE_PATH}")
    ...
    try:
        if verbose:
            print("[v16] cargando quarter scores...")
        qs = _load_quarter_scores(conn)
        if verbose:
            print("[v16] cargando metadata...")
        ...
        if verbose:
            print(f"[v16] {len(matches)} partidos completos (genero filtrado)")
        ...
        if verbose:
            print(f"[v16] {len(samples)} muestras cacheadas en {CACHE_PATH}")
