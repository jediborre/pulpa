...
def build_samples(
    ...
    if use_cache and CACHE_PATH.exists():
        if verbose:
            print(f"[v15] usando cache {CACHE_PATH}")
    ...
    try:
        if verbose:
            print("[v15] cargando quarter scores...")
        qs = _load_quarter_scores(conn)
        if verbose:
            print("[v15] cargando metadata...")
        ...
        if verbose:
            print(f"[v15] {len(matches)} partidos completos (genero filtrado)")
        ...
        if verbose:
            print(f"[v15] {len(samples)} muestras cacheadas en {CACHE_PATH}")
