"""
test_roi.py - Script de prueba de ROI V15 con salida legible.
...
Uso:
    python -m training.v15.cli test-roi
    python -m training.v15.cli test-roi --odds 1.50 --top 10
    python -m training.v15.test_roi   # directo

Salidas:
  reports/test_roi_v15.csv  (tabla completa por liga+target)
  reports/test_roi_v15.json (resumen estructurado para consumir desde API)
"""
...
def run(
    ...
    if not SUMMARY_PATH.exists():
        print(f"{C.RED}[error] no hay modelos entrenados. "
              f"Corre: python -m training.v15.cli train{C.RESET}")
        return {}
    ...
    print(f"{C.BOLD}{C.CYAN}[v15/test-roi] odds={odds}  "
          f"break-even={1/odds:.4f}  target={config.TARGET_HIT_RATE:.0%}{C.RESET}")
    print(f"[v15/test-roi] cargando engine...")
    engine = V15Engine.load(SUMMARY_PATH)

    print(f"[v15/test-roi] cargando muestras y split...")
    ...
    if not holdout:
        ...
        return {}
    print(f"[v15/test-roi] {len(holdout)} muestras de holdout "
    ...
    print(f"[v15/test-roi] corriendo inferencia...")
    ...
    out_json = REPORTS_DIR / "test_roi_v15.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, default=_jsonable)
    print(f"\n[v15/test-roi] resumen -> {out_json}")

    if emit_csv:
        out_csv = REPORTS_DIR / "test_roi_v15.csv"
        ...
        print(f"[v15/test-roi] csv     -> {out_csv}")
...
def _print_header(odds: float, n_holdout: int, n_preds: int) -> None:
    ...
    print(f"{C.BOLD}{C.CYAN}  V15 - ROI TEST REPORT{C.RESET}")
    print(f"{C.BOLD}{'=' * 92}{C.RESET}")
...
def _print_hint(global_roi, portfolio_roi) -> None:
    ...
    if portfolio_roi is None:
        ...
        print(f"  - Correr {C.BOLD}python -m training.v15.cli plots{C.RESET} "
