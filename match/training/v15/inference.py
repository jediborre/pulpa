"""
inference.py - Motor de inferencia V15 (carga una vez, inferencia multiple).
...
from training.v15 import config, dataset as ds, features as feat, gates, league_overrides
...
class V15Engine:
    ...
    def load(cls, summary_path: Path | str = SUMMARY_PATH) -> "V15Engine":
        ...
        for m in summary.get("models", []):
            ...
            try:
                ...
                print(f"[v15] error cargando modelo {league}/{target}: {e}")
                continue
    ...
    def predict(
        ...
        debug = Debug(
            ...
            model_found=False,
            model_version=self.summary.get("version", "v15"),
        )
