"""
plots.py - Graficas de diagnostico V15.
...
from training.v15.inference import V15Engine, SUMMARY_PATH, MODEL_DIR
...
def _load_summary() -> dict:
    if not SUMMARY_PATH.exists():
        raise FileNotFoundError(
            f"No existe {SUMMARY_PATH}. Corre `python -m training.v15.cli train` primero."
        )
...
def generate_all(
    ...
    if include_inference_based:
        ...
        print("[v15/plots] 06 distribucion probabilidades...")
        paths.append(plot_probability_distribution(all_preds))

        if include_calibration:
            print("[v15/plots] 05 calibration curves...")
            paths.append(plot_calibration_curves(summary, engine, holdout_by_key, gp, pbp))

    print(f"[v15/plots] generadas {len(paths)} graficas en {PLOTS_DIR}")
    return paths
