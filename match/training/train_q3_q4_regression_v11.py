"""
V11 - Over/Under Regression Models for Q3/Q4 Total Points
Gender-separated training (men_or_open vs women)

Key changes from V10:
- Model predicts ACTUAL total points (e.g., 45 pts) - not threshold
- Separate models for men_or_open vs women (gender inference)
- No hardcoded thresholds - sportsbook line provided at prediction time
"""
...
def build_samples(target_quarter: str) -> list[OverUnderSample]:
    ...
    print(f"[V11] Building samples for {target_quarter}...")
    ...
    for row in rows:
        ...
        samples.append(OverUnderSample(match_id, dt, gender, f, target_home, target_away, target_total))
        
        team_history[ht].append(1 if score["home"] > score["away"] else 0)
        team_history[at].append(1 if score["away"] > score["home"] else 0)

    conn.close()
    print(f"[V11] Built {len(samples)} samples")
    return samples


def train_regression_gender(
    samples: list[OverUnderSample], 
    target_type: str, 
    target_name: str,
    gender: str
) -> dict:
    """Train regression model for specific gender and target"""
    print(f"[V11] Training {target_name}_{target_type} for {gender}...")
    ...
    if target_type == "home":
        targets = [s.target_home for s in samples_filtered]
    elif target_type == "away":
        targets = [s.target_away for s in samples_filtered]
    else:
        targets = [s.target_total for s in samples_filtered]
    ...
    metrics = {
        "model": f"{target_name}_{target_type}_{gender}",
        "gender": gender,
        "n_train": n_train,
        "n_test": n_test,
        "mae": round(mae, 2),
    ...
    for name, model in models.items():
        ...
        joblib.dump(artifact, OUT_DIR / f"{target_name}_{target_type}_{gender}_{name}.joblib")
    ...
    joblib.dump({
        ...
        "models": models,
        "has_xgb": HAS_XGB
    }, OUT_DIR / f"{target_name}_{target_type}_{gender}_ensemble.joblib")
    
    print(f"[V11] {target_name}_{target_type}_{gender} done - MAE: {mae:.2f}")
...
def main():
    ...
    genders = ["men_or_open", "women"]
    
    for target_name, samples in [("q3", samples_q3), ("q4", samples_q4)]:
        for target_type in ["home", "away", "total"]:
            for gender in genders:
                result = train_regression_gender(samples, target_type, target_name, gender)
    ...
    print(f"\n[V11] Done in {elapsed:.1f}s")
    print(f"[V11] Output: {OUT_DIR}")
