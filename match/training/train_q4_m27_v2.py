...
def _run_cv(
    ...
    hist_model.fit(x_train.toarray() if hasattr(x_train, "toarray") else x_train, y_train)
    ...
    hist_val_prob = hist_model.predict_proba(
        x_val.toarray() if hasattr(x_val, "toarray") else x_val
    )[:, 1]
    hist_test_prob = hist_model.predict_proba(
        x_test.toarray() if hasattr(x_test, "toarray") else x_test
    )[:, 1]

    ens_train_prob = 0.5 * xgb_train_prob + 0.5 * hist_train_prob
    ens_val_prob = 0.5 * xgb_val_prob + 0.5 * hist_val_prob
    ens_test_prob = 0.5 * xgb_test_prob + 0.5 * hist_test_prob
